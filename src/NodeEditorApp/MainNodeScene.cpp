...
namespace node
{
struct SceneIdFixer : loader::BlockModifier
{
public:
    SceneIdFixer(SceneManagerId id) : m_Sceneid{ id } {}
    SceneManagerId m_Sceneid;
    void ModifyBlock(node::model::BlockModel& block) override
    {
        auto& properties = block.GetProperties();
        if (block.GetClass() == "SubSystem")
        {
            auto it = std::find_if(properties.begin(), properties.end(), [&](const model::BlockProperty& p) { return p.name == "SceneId"; });
            if (it == properties.end())
            {
                assert(false);
                return;
            }
            it->prop = static_cast<uint64_t>(m_Sceneid.value);
        }
    }
};
}
...
LoadSceneAndSubscenes(node::loader::SceneLoader& db_conn, node::SubSceneId id, node::loader::BlockModifier* modifier)
{
    using namespace node;
    std::unordered_map<SubSceneId, std::shared_ptr<model::NodeSceneModel>> scenes;
    SDL_Log("Loading Scene %d", id.value);
    auto scene = db_conn.Load(id, modifier);
    if (!scene)
    {
    ...
    {
        auto child_scenes = LoadSceneAndSubscenes(db_conn, child_id, modifier);
        if (!child_scenes)
        {
            return std::nullopt;
        }
...
void node::MainNodeScene::LoadScene(std::string name)
{
    ...
    auto manager_id = GetNewManagerId();
    SceneIdFixer id_fixer{ manager_id };
    if (auto new_scenes = LoadSceneAndSubscenes(*connector.connector,SubSceneId{1}, &id_fixer))
    {
        CloseAllDialogs();
        CreateNewScene(manager_id);
        assert(m_sceneComponents.find(manager_id) != m_sceneComponents.end());
...
void node::MainNodeScene::CloseTabRequest(size_t tab_idx)
{
    ...
    {
        SDL_Log("Closing Tab with only 1 left");
        return;
    }

    assert(tab_idx < m_sceneComponents.size());
    if (!(tab_idx < m_sceneComponents.size()))
    {
        return;
    }
...
void node::MainNodeScene::InitializeSidePanel()
{
    ...
    std::vector<model::BlockProperty>{
            ...
            node::model::BlockProperty{"SceneId", node::model::BlockPropertyType::UnsignedInteger, uint64_t{0}},

...
struct TestExecutor : public async::Executor
{
	...
	void Post(Task task) override
	{
		m_tasks.push_back(task.Release());
		m_tasks.back().promise().SetExecutor(this);
		made_ready.push_back(m_tasks.back());
	}
	...
	void RemoveTask(std::coroutine_handle<Task::promise> h) override
	{
	...
	std::vector<std::coroutine_handle<Task::promise>> m_tasks;
...
TEST(testAsyncTask, testSleep)
{
	...
	ASSERT_TRUE(executor.m_tasks[0]);
...
TEST(testAsyncTask, testAwaitAnotherTask)
{
	...
	ASSERT_EQ(executor.m_tasks.size(), 2);
	EXPECT_TRUE(executor.passed_delay);
	EXPECT_EQ(*executor.passed_delay, 3);
	ASSERT_EQ(executor.made_ready.size(), 1);
	EXPECT_EQ(executor.m_tasks[1].address(), executor.made_ready[0].address());

	ASSERT_FALSE(executor.m_tasks[1].done());
	executor.m_tasks[1].resume();
...
TEST(testAsyncTask, testExceptionPropagation)
{
	...
	ASSERT_EQ(executor.m_tasks.size(), 1);
	ASSERT_TRUE(executor.m_tasks[0].promise().get_error());
	std::string exception_what;
	try
	{
		std::rethrow_exception(executor.m_tasks[0].promise().get_error());
	}

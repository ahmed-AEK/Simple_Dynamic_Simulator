...
namespace detail
{
	constexpr auto NLStatefulEqDeleter = [](INLStatefulEquation* ptr) { if (ptr) { ptr->Destroy(); } };
}

using NLStatefulEqPtr = std::unique_ptr<INLStatefulEquation, decltype(detail::NLStatefulEqDeleter)>;

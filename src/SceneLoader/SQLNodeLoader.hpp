...
public:
	...
	bool AddNode(const node::model::NodeModel& node) override;
	...
	bool UpdateNodePosition(node::model::id_int node_id,
		...
		std::optional<node::model::NodeModel> GetNode(
			node::model::id_int node_id) override;
	...
	std::vector<node::model::NodeModel> GetNodes() override;

...
{
	m_buttons.push_back(std::make_unique<DialogButton>(std::move(title), std::move(callback), WidgetSize{ 80,ButtonHeight }, this));
	ResizeToFitChildren();
}
...
node::DialogButton::DialogButton(std::string text, std::function<void()> OnClick, 
	const WidgetSize& size, Widget* scene)
	:Widget{size, scene}, m_text{text}, m_onClick{OnClick}
{
	SetFocusable(true);
}
...
void node::DialogButton::OnDraw(SDL::Renderer& renderer)
{
	...
	text_rect.x = (GetSize().w - text_rect.w) / 2;
	text_rect.y = (GetSize().h - text_rect.h) / 2;

...
public:
	...
	virtual void RemoveTask(std::coroutine_handle<Task::promise> h) = 0;
...
public:
	...
	void await_resume() {
		...
		if (task_to_remove)
		{
			if (auto error_ptr = task_to_remove.promise().get_error())
			{
				error = std::move(error_ptr);
			}
			executor->RemoveTask(task_to_remove);
			if (error)
			{
	...
	Executor* executor;
	Task task;
	std::coroutine_handle<Task::promise> task_to_remove;
};

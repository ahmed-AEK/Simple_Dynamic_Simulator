...
{
	SDL_Log("Close BotPanel Pressed!");
	m_bot_panel_visibility.last_action_time = SDL_GetTicks();
	if (!m_bot_panel_visibility.updateTaskId)
	{
...
void node::SceneGrid::RepositionWidgets()
{
	if (m_side_panel)
	{
		m_side_panel->SetSize({ std::min(GetSize().w - 50, m_side_panel_preferred_width), GetSidePanelExpectedHeight() });
		m_side_panel->SetPosition(GetSidePanelPosition());
	}
	if (m_bot_panel)
	{
		m_bot_panel->SetSize({ GetBotPanelExpectedWidth(), std::min(GetSize().h, m_bot_panel_preferred_height) });

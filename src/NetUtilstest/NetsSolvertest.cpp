...
TEST(testNetsSolver, test_nShape2)
{
	NetsSolver solver;
	node_int extension_distance = 40;
	solver.SetExtensionDistance(extension_distance);
	NetSolutionEndDescription node1{ model::Point{150,89}, {true,false,false,false} };
	NetSolutionEndDescription node2{ model::Point{322,97}, {true,true,true,true} };
	solver.SetStartDescription(node1);
	solver.SetEndDescription(node2);
	auto solution1 = solver.Solve();
	solver.Reset();

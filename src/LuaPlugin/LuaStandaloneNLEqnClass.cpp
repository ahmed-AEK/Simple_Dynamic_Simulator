#include "LuaStandaloneNLEqnClass.hpp"
#include "sol/sol.hpp"
...
const std::string_view node::LuaStandaloneNLEqnClass::DEFAULT_CODE = R"(
...
node::LuaStandaloneNLEqnClass::LuaStandaloneNLEqnClass()
	:node::BlockClass{""}
{
}

void node::LuaStandaloneNLEqnClass::GetName(GetNameCallback cb, void* context) const
{
	static constexpr std::string_view name = "LuaStandaloneNLEqn";
	cb(context, name);
}

void node::LuaStandaloneNLEqnClass::GetDescription(GetDescriptionCallback cb, void* context) const
{
	cb(context, Description);
}

void node::LuaStandaloneNLEqnClass::GetDefaultClassProperties(GetDefaultClassPropertiesCallback cb, void* context) const
{
	cb(context, ClassProperties);
}

int node::LuaStandaloneNLEqnClass::ValidateClassProperties(std::span<const model::BlockProperty> properties, IValidatePropertiesNotifier& error_cb) const
{
...
void node::LuaStandaloneNLEqnClass::CalculateSockets(std::span<const model::BlockProperty> properties, CalculateSocketCallback cb, void* context) const
{
...
node::BlockType node::LuaStandaloneNLEqnClass::GetBlockType(std::span<const model::BlockProperty> properties) const
{
	UNUSED_PARAM(properties);
	return BlockType::Stateful;
}

int node::LuaStandaloneNLEqnClass::GetFunctor(std::span<const model::BlockProperty> properties, IGetFunctorCallback& cb) const
{	
...
bool node::LuaStandaloneNLEqnClass::HasBlockDialog() const
{
	return true;
}
...
std::unique_ptr<node::BlockDialog> node::LuaStandaloneNLEqnClass::CreateBlockDialog(Scene& scene, std::shared_ptr<IBlockPropertiesUpdater> model_updater, model::BlockModel& model, model::FunctionalBlockData& data, std::any& simulation_data)
{
	UNUSED_PARAM(data);
...
tl::expected<std::monostate, std::string> node::LuaStandaloneNLEqnClass::validate_lua_script(const std::string& code) const
{
	lua::NLEqnBuilder builder{ m_logger };

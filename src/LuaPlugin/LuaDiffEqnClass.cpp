...
static constexpr std::string_view Description = "Loads a lua file from Path that is a valid DiffEqn";

void node::LuaDiffEqnClass::GetName(GetNameCallback cb, void* context) const
{
	static constexpr std::string_view name = "LuaDiffEqn";
	cb(context, name);
}

void node::LuaDiffEqnClass::GetDescription(GetDescriptionCallback cb, void* context) const
{
	cb(context, Description);
}

void node::LuaDiffEqnClass::GetDefaultClassProperties(GetDefaultClassPropertiesCallback cb, void* context) const
{
	cb(context, ClassProperties);
}

int node::LuaDiffEqnClass::ValidateClassProperties(std::span<const model::BlockProperty> properties, IValidatePropertiesNotifier& error_cb) const
{
...
void node::LuaDiffEqnClass::CalculateSockets(std::span<const model::BlockProperty> properties, CalculateSocketCallback cb, void* context) const
{
...
node::BlockType node::LuaDiffEqnClass::GetBlockType(std::span<const model::BlockProperty> properties) const
{
	UNUSED_PARAM(properties);
	return BlockType::Stateless;
}

int node::LuaDiffEqnClass::GetFunctor(std::span<const model::BlockProperty> properties, IGetFunctorCallback& cb) const
{
...
tl::expected<std::monostate, std::string> node::LuaDiffEqnClass::validate_lua_script(std::string_view path) const
{
	lua::NLEqnBuilder builder{m_logger};
